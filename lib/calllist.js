...
const callLists = function () {
   ...
   /* BEGIN: Read TAM status */
   function TAM_GetMessageList(){
   let parseString = require('xml2js').parseString;
   let request = require('request');
   let Result_Fritzbox_HyperlinkXmlTAM;
//   let Fritzbox_AnrufbeantworterAnzahlNeueNachrichtenn = 0;
//   let Fritzbox_AnrufbeantworterDaten_json = "";
   let befehl_GetMessageList = '{"service": "urn:dslforum-org:service:X_AVM-DE_TAM:1","action": "GetMessageList","params": {"NewIndex ": "' + abIndex + '"}}';
   setState("tr-064.0.states.command","{}");
   setState("tr-064.0.states.command",befehl_GetMessageList); 
   adapter.log.debug('tr-064.0.states.commandResult: ' + getState('tr-064.0.states.commandResult').val);
//   setState(DP_Fritzbox_AnrufbeantworterIndexMessage_json,"");  //Setzt den aktuellen Inhalt vom Datenpunkt zurück, damit im Verlauf die Index Nummer von den Anrufen neu geschrieben werden können
   ...
   Result_Fritzbox_HyperlinkXmlTAM = Result_Fritzbox_HyperlinkXmlTAM.substring(11, getState("tr-064.0.states.commandResult").val.length -2);  
   adapter.log.debug('XML-Link: " + Result_Fritzbox_HyperlinkXmlTAM);
   request(Result_Fritzbox_HyperlinkXmlTAM, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            adapter.log.debug('FB_xml_TamCalls: ' + FB_xml_TamCalls);
            setState(abNumofmessages, FB_xml_TamCalls);
  ...
  /* END: Read TAM status */
    ...
    this.addCall2List = function (call, listName) {
        ...
        if (!list.array.find(function (v) {
            ...
            if (list.array.length > list.cfg.maxEntries) {
                list.array.length = list.cfg.maxEntries;
            }
...
function refresh(err, data, cb, done) {
    ...
    getHttpData(url, {xml2json: true}, (err, json) => {
        ...
        json && json.root && systemData.native.callLists.add(json.root.call, ~~json.root.timestamp);
